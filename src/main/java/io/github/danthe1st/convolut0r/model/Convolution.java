...
public record Convolution(double[][][] convolution, int striding, int padding) {// NOSONAR I don't care about hashCode/equals
	...
	public int[][][] convolute(int[][][] pixels) {
		...
		for(int channel = 0; channel < ret.length; channel++){
			for(int oldI = -padding, newI = 0; oldI + convolution[0].length < pixels[0].length + padding; oldI += striding, newI++){
				for(int oldJ = -padding, newJ = 0; oldJ + convolution[0][0].length < pixels[0][0].length + padding; oldJ += striding, newJ++){
					double sum = 0;
